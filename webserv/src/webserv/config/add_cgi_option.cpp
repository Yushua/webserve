...
void webserv::config_add_cgi_option(const string extension, const string interpreter_path, int line) {
	...
	if (found != cgi_options.end()) {
		std::cerr << RED << "  -~={ line " << line << ": cgi for ." << extension << " already exists }=~-\n" << RESET;
		return;
	}
	...
	if (interpreter_path != "") {
		...
		if (stat(interpreter_path.c_str(), &file_info) == -1) {
			std::cerr << RED << "  -~={ line " << line << ": cgi: path " << interpreter_path << " doesn't exist }=~-\n" << RESET;
			return;
		}
		if (!S_ISREG(file_info.st_mode)) {
			std::cerr << RED << "  -~={ line " << line << ": cgi: " << interpreter_path << " is not a file }=~-\n" << RESET;
			return;
		}
	}
